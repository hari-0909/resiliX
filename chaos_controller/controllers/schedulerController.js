const {listPods}=require("../services/kubeService")
...
async function scheduleChaos(req,res){
  try{
    ...
    const experiment=await createPodDeleteExperiment(service,"default","scheduled")
    ...
    try{
      ...
      if(targetPod){
        await queuePodDeleteExecution(experiment,"schedule")
        console.log(`scheduled chaos queued for ${targetPod.metadata.name}`)
      }
    }catch(err){
      console.log("scheduler error",err)
    }
    },interval*1000)
    activeSchedules[service]={job,interval,experimentId:experiment.id}
    res.json({
